...
function exportarCSV(dados: Record<string, string>[]) {
  ...
  a.download = `matriz-competencias-${new Date().toLocaleDateString('pt-BR').replace(/\//g, '-')}.csv`
...
function exportarXLSX(dados: Record<string, string>[]) {
  ...
  import('xlsx').then(XLSX => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Matriz de Competências')
    XLSX.writeFile(wb, `matriz-competencias-${new Date().toLocaleDateString('pt-BR').replace(/\//g, '-')}.xlsx`)
  })
}
...
function CelulaExameNR({ registro, semPrazo, onClick, onClickIcone, compacto }: {
  ...
  if (!registro) {
    return (
      <td style={{ padding: pad, textAlign: 'center', cursor: 'pointer', backgroundColor: '#f9f9f9', color: '#ccc', fontSize: 12, minWidth: minW, verticalAlign: 'middle' }} onClick={onClick}>—</td>
    )
  }
            ...
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
              <div style={{ backgroundColor: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 440, boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
                ...
                <p style={{ fontSize: 13, color: '#555', margin: '0 0 16px', lineHeight: 1.5 }}>
                  Esta ação é <strong>irreversível</strong>. Para confirmar, digite o nome da NR:
                </p>
...
export default function MatrizCompetenciasPage() {
  ...
  async function buscarColaboradores() {
    ...
    if (filtroGerencia) query = query.eq('gerencia_id', filtroGerencia)
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <h1 style={{ fontSize: 18, fontWeight: 600, color: '#1a1a1a', margin: '0 0 12px' }}>Matriz de Competências</h1>
