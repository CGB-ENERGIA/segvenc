...
// Exames complementares — nome curto → nome completo no banco
const EXAMES_COMPL = [
...
interface ASO {
  id: string; tipo: string; data_realizacao: string
  data_vencimento: string | null; gse: number | null; observacao: string | null
  url_arquivo: string | null; logs_auditoria: LogAuditoria[]; programacoes: ProgramacaoASO[]
}

interface ExameCompl {
  id: string; tipo_exame_id: number; nome_exame: string
  data_realizacao: string; url_arquivo: string | null
}

interface Colaborador {
  matricula: string; nome: string; situacao: string
  funcao_id: number | null; gse: number | null
  data_admissao: string | null; processo: string | null
  bases: { nome: string } | null; funcoes: { nome: string } | null
  gerencias: { sigla: string } | null
  asos: ASO[]
  exames_compl: ExameCompl[]
}
...
// Lógica principal: ASO mais recente para status
// PER, RET, MRO → usa data_vencimento
// ADM → +1 ano da realização como vencimento
// DEM → não conta
function getASOPrincipal(col: Colaborador): { aso: ASO; vencimento: string } | null {
  ...
  if (ret) {
    const d = new Date(ret.data_realizacao + 'T12:00:00')
    d.setFullYear(d.getFullYear() + 1)
    candidatos.push({ aso: ret, vencimento: d.toISOString().split('T')[0], peso: 2 })
  }
  if (mro) {
    const d = new Date(mro.data_realizacao + 'T12:00:00')
    d.setFullYear(d.getFullYear() + 1)
    candidatos.push({ aso: mro, vencimento: d.toISOString().split('T')[0], peso: 2 })
  }
  if (adm && candidatos.length === 0) {
    const d = new Date(adm.data_realizacao + 'T12:00:00')
    d.setFullYear(d.getFullYear() + 1)
    candidatos.push({ aso: adm, vencimento: d.toISOString().split('T')[0], peso: 1 })
  }
  if (!candidatos.length) return null
  // Pega o com maior vencimento entre os de mesmo peso máximo
...
function getStatusColaborador(col: Colaborador): StatusASO {
  ...
  if (!principal) return 'sem_aso'
  return getStatusASO(principal.vencimento)
}

function temProgramado(col: Colaborador): boolean {
  return col.asos.some(a => (a.programacoes || []).length > 0)
}
...
function gerarExport(colabs: Colaborador[]) {
  return colabs.map(c => {
    ...
    const dias = principal ? calcularDias(principal.vencimento) : null
    const row: Record<string, string | number> = {
      'Matrícula': c.matricula, 'Nome': c.nome,
      'Base': c.bases?.nome || '', 'Situação': c.situacao,
      'Admissão': formatarData(c.data_admissao),
      'Função': c.funcoes?.nome || '', 'Processo': c.processo || '',
      'GSE': c.gse ?? '',
      'ASO': principal ? tipoSigla(principal.aso.tipo) : '—',
      'Dias Restantes': dias ?? '',
    ...
    EXAMES_COMPL.forEach(e => {
      const ex = c.exames_compl.find(x => x.nome_exame === e.nome)
      row[e.label] = formatarData(ex?.data_realizacao)
    })
    return row
  })
}
...
function Icone({ tipo, cor, titulo, size = 16 }: { tipo: string; cor: string; titulo?: string; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={cor} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline-block', verticalAlign: 'middle', flexShrink: 0 }}>
      {titulo && <title>{titulo}</title>}<path d={paths[tipo] || ''} />
    </svg>
  )
}

// ─── CÉLULA ASO PRINCIPAL ─────────────────────────────────────────────────────
function CelulaASOPrincipal({ colab, onClick, onCalendario, compacto }: {
  colab: Colaborador; onClick: () => void; onCalendario: () => void; compacto: boolean
}) {
  const principal = getASOPrincipal(colab)
  const status = getStatusColaborador(colab)
  ...
  if (!principal) return (
    <td style={{ ...base, backgroundColor: '#f1f5f9' }}>
      <span style={{ fontSize: 11, color: '#94a3b8', fontStyle: 'italic' }}>Sem ASO</span>
    </td>
  )

  const { aso, vencimento } = principal
    ...
    <td style={{ ...base, backgroundColor: bgMap[status] }} onClick={onClick}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
        ...
        <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
          ...
          {dias !== null && dias <= 60 && (
            <span title={prog ? `Prog: ${formatarData(prog.data_programada)}` : 'Não programado'}
              onClick={e => { e.stopPropagation(); onCalendario() }} style={{ cursor: 'pointer', display: 'flex' }}>
              <Icone tipo="calendario" cor={prog ? '#7c3aed' : '#9ca3af'} size={12} />
            </span>
          )}
        </div>
...
function CelulaDias({ colab, compacto }: { colab: Colaborador; compacto: boolean }) {
  ...
  const dias = calcularDias(principal.vencimento)
  if (dias === null) return <td style={{ ...base, color: '#ccc' }}>—</td>
  const bgMap: Record<StatusASO, string> = { no_prazo: '#f0fdf4', critico: '#fefce8', atencao: '#fff7ed', vencido: '#fef2f2', sem_aso: '#f1f5f9' }
  return (
    <td style={{ ...base, backgroundColor: bgMap[status] }}>
      <span style={{ fontSize: compacto ? 12 : 13, fontWeight: 700, color: cores.text }}>
...
// ─── CÉLULA ASO DETALHADO (expandido) ────────────────────────────────────────
function CelulaASODetalhe({ aso, tipo, onClick, onCalendario, compacto, isMaisRecente }: {
  aso: ASO | null; tipo: string; onClick: () => void; onCalendario: () => void; compacto: boolean; isMaisRecente: boolean
}) {
  const pad = compacto ? '6px 8px' : '8px 12px'
  ...
  if (!aso) return (
    <td style={{ ...base }} onClick={onClick}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        <Icone tipo="plus" cor="#ddd" size={11} />
  ...
  const aud = [...(aso.logs_auditoria || [])].sort((a, b) => new Date(b.data_auditoria).getTime() - new Date(a.data_auditoria).getTime())[0]
  const prog = [...(aso.programacoes || [])].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0]
  const data = semVenc ? aso.data_realizacao : aso.data_vencimento
  const dias = semVenc ? null : calcularDias(aso.data_vencimento)
  return (
    <td style={{ ...base, backgroundColor: isMaisRecente ? '#f8f8f8' : '#fafafa' }} onClick={onClick}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          {dias !== null && dias <= 60 && (
            <span title={prog ? `Prog: ${formatarData(prog.data_programada)}` : 'Não programado'}
              onClick={e => { e.stopPropagation(); onCalendario() }} style={{ cursor: 'pointer', display: 'flex' }}>
              <Icone tipo="calendario" cor={prog ? '#7c3aed' : '#d0d0d0'} size={11} />
            </span>
          )}
        </div>
      </div>
...
// ─── CÉLULA EXAME COMPLEMENTAR ────────────────────────────────────────────────
function CelulaExameCompl({ exame, onClick, compacto }: {
  exame: ExameCompl | undefined; onClick: () => void; compacto: boolean
}) {
  const pad = compacto ? '6px 8px' : '8px 12px'
  ...
  return (
    <td style={{ ...base, backgroundColor: '#f8f8f8' }} onClick={onClick}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
...
function ModalASO({ dados, abaInicial, onClose, onUpdate, email, podeAuditar, nivel }: {
  ...
  abaInicial: AbaModal; onClose: () => void; onUpdate: () => void
  email: string; podeAuditar: boolean; nivel: string
}) {
  const [aba, setAba] = useState<AbaModal>(abaInicial)
  const [arquivo, setArquivo] = useState<File | null>(null)
  const [uploading, setUploading] = useState(false); const [errUp, setErrUp] = useState('')
  const [formAud, setFormAud] = useState({ validado: true, observacao: '' })
  const [salvAud, setSalvAud] = useState(false); const [errAud, setErrAud] = useState('')
  const [modalExc, setModalExc] = useState(false); const [confNome, setConfNome] = useState('')
  const [excluindo, setExcluindo] = useState(false); const [errExc, setErrExc] = useState('')
  ...
  const [salvProg, setSalvProg] = useState(false); const [errProg, setErrProg] = useState('')
  ...
  async function salvarProg() {
    ...
    const { data, error } = await supabase.from('programacoes_exames').insert({
      aso_id: aso.id, matricula_colaborador: colab.matricula,
      data_programada: formProg.data_programada, observacao: formProg.observacao || null, criado_por: email,
    }).select().single()
    if (error) { setErrProg(error.message); setSalvProg(false); return }
    setProgs(p => [{ ...data }, ...p])
    setFormProg({ data_programada: '', observacao: '' }); setSalvProg(false); onUpdate()
  }
  ...
  async function salvarAud() {
    ...
    const { error } = await supabase.from('logs_auditoria').insert({
      ...
      validado: formAud.validado, observacao: formAud.observacao || null, data_auditoria: new Date().toISOString(),
    })
              ...
              {!semVenc && aso.data_vencimento && (
                <div style={{ padding: '10px 14px', borderRadius: 10, marginBottom: 16, backgroundColor: cores.bg, border: `1px solid ${cores.text}30` }}>
                  ...
                  {calcularDias(aso.data_vencimento) !== null && (
                    <span style={{ fontSize: 12, color: cores.text, marginLeft: 8 }}>({calcularDias(aso.data_vencimento)! > 0 ? `${calcularDias(aso.data_vencimento)} dias restantes` : `${Math.abs(calcularDias(aso.data_vencimento)!)} dias vencido`})</span>
                  )}
                </div>
              )}
                ...
                : progs.map((p, i) => (
                  <div key={p.id} style={{ border: '1px solid #f0f0f0', borderRadius: 8, padding: '10px 14px', marginBottom: 8, borderLeft: '3px solid #7c3aed' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#7c3aed' }}>📅 {formatarData(p.data_programada)}{i === 0 && <span style={{ fontSize: 10, color: '#aaa', marginLeft: 8 }}>mais recente</span>}</span>
...
function ModalExameCompl({ colab, nomeExame, exame, onClose, onUpdate }: {
  colab: Colaborador; nomeExame: string; exame: ExameCompl | undefined
  ...
  async function salvar() {
    ...
    try {
      ...
      if (exame) {
        await supabase.from('exames_aso').update({ data_realizacao: dataRealizacao, url_arquivo: urlArquivo }).eq('id', exame.id)
      } else {
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}>
      <div style={{ backgroundColor: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: '#1a1a1a', margin: 0 }}>{nomeExame}</h2>
            <p style={{ fontSize: 13, color: '#888', margin: '3px 0 0' }}>{colab.nome} · {colab.matricula}</p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed #e0e0e0', borderRadius: 10, padding: '16px 20px', textAlign: 'center', cursor: 'pointer', backgroundColor: '#fafafa' }}>
              <Icone tipo="upload" cor="#aaa" size={20} />
              <p style={{ fontSize: 12, color: '#888', margin: '6px 0 0' }}>{arquivo ? arquivo.name : exame?.url_arquivo ? 'Substituir documento' : 'Clique para anexar'}</p>
              <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }} onChange={e => setArquivo(e.target.files?.[0] || null)} />
            </div>
          </div>
...
// ─── MODAL NOVO ASO ───────────────────────────────────────────────────────────
// Tipos auxiliares para exames no modal
...
function ModalNovoASO({ colab, onClose, onSalvo }: { colab: Colaborador; onClose: () => void; onSalvo: () => void }) {
  ...
  // Mapa tipo ASO → coluna na gse_exames
  const CAMPO_TIPO: Record<string, string> = {
    admissional: 'no_adm', periodico: 'no_per',
    retorno: 'no_ret', mudanca_risco: 'no_mro', demissional: 'no_dem',
  }
  ...
  // Buscar todos os tipos de exame (para o select de extras)
  useEffect(() => {
    supabase.from('tipos_exame_medico').select('id, nome').order('nome').then(({ data }) => setTodosExames(data || []))
  }, [])

  // Buscar exames sugeridos quando tipo ou gse mudam
  useEffect(() => {
    async function buscarExames() {
      if (!colab.gse) return
      const campo = CAMPO_TIPO[tipo]
      if (!campo) { setExamesSugeridos([]); return }
      setLoadingExames(true)

      // Buscar exames do GSE para esse tipo
      const { data: gseEx } = await supabase
        .from('gse_exames')
        .select(`tipo_exame_id, ${campo}, tipos_exame_medico(id, nome)`)
        .eq('gse_id', colab.gse)
        .eq(campo, true)

      if (!gseEx?.length) { setExamesSugeridos([]); setLoadingExames(false); return }

      // Buscar últimas datas dos exames do colaborador
      const tipoIds = gseEx.map((g: any) => g.tipo_exame_id)
      const { data: ultimos } = await supabase
        .from('exames_aso')
        .select('tipo_exame_id, data_realizacao')
        .eq('matricula_colaborador', colab.matricula)
        .in('tipo_exame_id', tipoIds)
        .order('data_realizacao', { ascending: false })

      // Montar mapa de última data por tipo
      const ultimoMap: Record<number, string> = {}
      ;(ultimos || []).forEach((u: any) => {
        if (!ultimoMap[u.tipo_exame_id]) ultimoMap[u.tipo_exame_id] = u.data_realizacao
      })

      const sugestoes: ExameSugerido[] = gseEx.map((g: any) => ({
        tipo_exame_id: g.tipo_exame_id,
        nome: g.tipos_exame_medico?.nome || '',
        data: dataRealizacao || '',
        marcado: true,
        ultimaData: ultimoMap[g.tipo_exame_id] || null,
      }))

      setExamesSugeridos(sugestoes)
      setLoadingExames(false)
    }
    buscarExames()
  }, [tipo, colab.gse])

  // Atualizar data dos exames quando dataRealizacao mudar
  useEffect(() => {
    if (!dataRealizacao) return
    setExamesSugeridos(prev => prev.map(e => ({ ...e, data: e.data || dataRealizacao })))
  }, [dataRealizacao])

  function toggleExame(idx: number) {
    setExamesSugeridos(prev => prev.map((e, i) => i === idx ? { ...e, marcado: !e.marcado } : e))
  }
  function setDataExame(idx: number, data: string) {
    setExamesSugeridos(prev => prev.map((e, i) => i === idx ? { ...e, data } : e))
  }
  function adicionarExtra() {
    if (!exameExtra) return
    const te = todosExames.find(t => t.id === parseInt(exameExtra))
    if (!te) return
    if (examesSugeridos.find(e => e.tipo_exame_id === te.id)) return
    setExamesSugeridos(prev => [...prev, { tipo_exame_id: te.id, nome: te.nome, data: dataRealizacao || '', marcado: true, ultimaData: null }])
    setExameExtra('')
  }
  function removerExtra(idx: number) {
    setExamesSugeridos(prev => prev.filter((_, i) => i !== idx))
  }
  ...
  async function salvar() {
    ...
    try {
      ...
      // Inserir ASO
      const { data: novoASO, error: insertErr } = await supabase.from('asos').insert({
        ...
        gse: colab.gse || null, observacao: observacao || null, url_arquivo: urlArquivo,
      }).select().single()
      if (insertErr) throw insertErr

      // Inserir exames complementares marcados
      const examesMarcados = examesSugeridos.filter(e => e.marcado && e.data)
      if (examesMarcados.length > 0 && novoASO) {
        const { error: exErr } = await supabase.from('exames_aso').insert(
          examesMarcados.map(e => ({
            aso_id: novoASO.id,
  ...
  // Exames extras = os que não vieram do GSE (adicionados manualmente)
  const idsSugeridos = new Set(examesSugeridos.slice(0, examesSugeridos.findIndex((_, i) => i >= (examesSugeridos.length))).map(e => e.tipo_exame_id))
      ...
      <div style={{ backgroundColor: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 540, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: semVencimento ? '1fr' : '1fr 1fr', gap: 12 }}>
            <div><label style={{ fontSize: 12, color: '#666', display: 'block', marginBottom: 4 }}>Data de realização *</label>
              <input type="date" value={dataRealizacao} onChange={e => setDataRealizacao(e.target.value)} style={inp} />
            </div>
            {!semVencimento && <div>
              <label style={{ fontSize: 12, color: '#666', display: 'block', marginBottom: 4 }}>{tipo === 'periodico' ? 'Vencimento (auto +1 ano)' : 'Vencimento'}</label>
              <input type="date" value={dataVencimento} onChange={e => { setDataVencimento(e.target.value); setVencimentoEditado(true) }} style={{ ...inp, backgroundColor: dataVencimento ? '#f0fdf4' : 'white' }} />
          ...
          {colab.gse && (
            <div style={{ border: '1px solid #e0e0e0', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', backgroundColor: '#f9f9f9', borderBottom: '1px solid #e0e0e0' }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#333', margin: 0 }}>Exames Complementares</p>
                <p style={{ fontSize: 11, color: '#888', margin: '2px 0 0' }}>Baseado no GSE {colab.gse} — desmarque os que não serão realizados</p>
              </div>

              {loadingExames ? (
                <div style={{ padding: '16px 14px', color: '#aaa', fontSize: 13 }}>Carregando exames...</div>
              ) : examesSugeridos.length === 0 ? (
                <div style={{ padding: '16px 14px', color: '#aaa', fontSize: 13 }}>Nenhum exame obrigatório para este tipo de ASO</div>
              ) : (
                  ...
                  {examesSugeridos.map((e, idx) => (
                    <div key={e.tipo_exame_id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', borderBottom: idx < examesSugeridos.length - 1 ? '1px solid #f5f5f5' : 'none', opacity: e.marcado ? 1 : 0.4 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 12, fontWeight: 500, color: '#333', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.nome}</p>
                        {e.ultimaData && <p style={{ fontSize: 11, color: '#aaa', margin: '1px 0 0' }}>Último: {new Date(e.ultimaData + 'T12:00:00').toLocaleDateString('pt-BR')}</p>}
                      </div>
                      <input type="date" value={e.data} onChange={ev => setDataExame(idx, ev.target.value)}
                        disabled={!e.marcado}
                        style={{ height: 32, border: '1px solid #e0e0e0', borderRadius: 6, padding: '0 8px', fontSize: 12, outline: 'none', backgroundColor: e.marcado ? 'white' : '#f5f5f5', width: 130, flexShrink: 0 }} />
                      {/* Botão remover apenas para extras */}
                      {!EXAMES_COMPL.find(ec => todosExames.find(t => t.id === e.tipo_exame_id && ec.nome === t.nome)) && (
                        <button onClick={() => removerExtra(idx)} style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', fontSize: 16, padding: '0 4px', flexShrink: 0 }}>✕</button>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {/* Adicionar exame extra */}
              <div style={{ padding: '10px 14px', borderTop: '1px solid #f0f0f0', display: 'flex', gap: 8 }}>
                <select value={exameExtra} onChange={e => setExameExtra(e.target.value)}
                  style={{ flex: 1, height: 34, border: '1px solid #e0e0e0', borderRadius: 8, padding: '0 10px', fontSize: 12, outline: 'none', backgroundColor: 'white', color: '#555' }}>
                  <option value="">+ Adicionar exame extra...</option>
                  {examesDisponiveis.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}
                </select>
                <button onClick={adicionarExtra} disabled={!exameExtra}
                  style={{ height: 34, padding: '0 14px', fontSize: 12, border: `1px solid ${COR}`, borderRadius: 8, backgroundColor: exameExtra ? COR : '#f5f5f5', color: exameExtra ? 'white' : '#aaa', cursor: exameExtra ? 'pointer' : 'not-allowed' }}>
                  Adicionar
                </button>
              </div>
            </div>
...
export default function MedTrabPage() {
    ...
    if (mats.length > 0) {
      ...
      for (let i = 0; i < mats.length; i += LOTE) {
        ...
        while (true) {
          ...
          asosData = [...asosData, ...aData]; if (aData.length < 500) break; fromA += 500
        }
      }
    }

    // Exames complementares em lotes
    let examesData: any[] = []
    if (mats.length > 0) {
      const LOTE = 100
      for (let i = 0; i < mats.length; i += LOTE) {
        ...
        while (true) {
          ...
          if (!eData || eData.length === 0) break
          examesData = [...examesData, ...eData]; if (eData.length < 500) break; fromE += 500
        }
      }
    }
  ...
  const stats = useMemo(() => {
    ...
    colabs.forEach(c => {
      ...
      if (s === 'no_prazo') r.no_prazo++; else if (s === 'critico') r.critico++
      else if (s === 'atencao') r.atencao++; else if (s === 'vencido') r.vencido++
  ...
  const ordenados = useMemo(() => [...filtrados].sort((a, b) => {
    ...
    switch (ordCol) {
      ...
      case 'gse': vA = a.gse ?? 9999; vB = b.gse ?? 9999; break
      case 'aso': {
        const pA = getASOPrincipal(a); const pB = getASOPrincipal(b)
        vA = pA ? pA.vencimento : '9999'; vB = pB ? pB.vencimento : '9999'; break
      }
      case 'dias': {
        const pA = getASOPrincipal(a); const pB = getASOPrincipal(b)
        vA = pA ? (calcularDias(pA.vencimento) ?? 9999) : 9999
        vB = pB ? (calcularDias(pB.vencimento) ?? 9999) : 9999; break
      }
    }
  ...
  // Cabeçalho separador para grupos expandidos
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button onClick={() => setExpandido(e => !e)} style={{ height: 36, padding: '0 12px', fontSize: 12, border: `1px solid ${expandido ? COR : '#e0e0e0'}`, borderRadius: 8, backgroundColor: expandido ? '#fdf2f5' : 'white', color: expandido ? COR : '#555', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          {expandido ? '⊟ Recolher' : '⊞ Expandir colunas'}
        </button>
          ...
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, backgroundColor: 'white', fontSize: fs }}>
            <thead>
              <tr style={{ backgroundColor: '#fafafa' }}>
                ...
                {expandido && <>
                  <th style={thSeparador('ADM')}>ADM</th>
                  <th style={thSeparador('PER')}>PER</th>
                  <th style={thSeparador('RET')}>RET</th>
                  <th style={thSeparador('MRO')}>MRO</th>
                  <th style={thSeparador('DEM')}>DEM</th>
